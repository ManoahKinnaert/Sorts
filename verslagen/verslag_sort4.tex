...
\urlstyle{same}
\title{SORT4: een korte analyse van Quicksort}
...
In dit (korte) verslag bespreken we een klein experimentje rond het sorteren van willekeurige reële getallen met behulp van Quicksort. Hierbij tellen we het aantal vergelijkingen tussen elementen alsook het aantal verwisselingen
...
We doen dit in de hoop een idee te krijgen van hoe Quicksort zich zal gedragen naarmate de invoergrootte steeds groter wordt. Daarnaast ook welke invloed de sturctuur van de data (of de lijst al dan niet gesorteerd is) heeft op het sorteren.
We onderzoeken dus de tijdscomplexiteit van Quicksort. Belangrijk te vermelden is dat we gebruik zullen maken van Hoare partitionering.
...
De specifieke waarden die in de arrays worden ingevuld zijn deze variëren van integer tot floating point getallen. Er wordt geen verschil verwacht qua tijdscomplexiteit
tussen arrays met floating point of integer waarden.
